...
viz1 + theme_classic() + labs(title="Distribución generacional de los diputados por tipo de elección", subtitle ="Legislatura LXV (2021-2024)", caption = "Fuente: Currícula de la Cámara de Diputados") +
  theme(legend.position = "top") + geom_text(aes(label = Total),  position = position_dodge(width = 1), vjust = -.4) + 
  scale_fill_jama(name = "Generación", labels = c("X", "Boomers", "Millennials", "Silenciosa", "Z"))+ scale_y_continuous(limits=c(0, 150)) + xlab("Tipo de elección")+
